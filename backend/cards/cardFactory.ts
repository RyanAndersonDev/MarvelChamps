    ...
    switch (blueprint.type) {
        ...
        case 'upgrade':
            return {
                ...
                thwMod: (blueprint as any).thwMod,
            } as Upgrade;
    ...
    switch (blueprint.type) {
        ...
        case 'support':
            return {
                ...
                exhausted: false,
                counters: blueprint.counters ?? 0,
                allyLimitBonus: (blueprint as any).allyLimitBonus,
            } as Support;
...
export function createSideScheme(cardId: number, instanceId: number) : SideScheme {
    ...
    if (!blueprint || blueprint.type !== "side-scheme")
        throw new Error(`Side scheme with ID ${cardId} not found in the map or is not a side-scheme.`)
